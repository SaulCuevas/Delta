...
class TOOL_C(QHBoxLayout):
    ...
    # ---- LAYOUT PRINCIPAL ----------------
        ...
        try:
            ...
            preview_config = self.picam2.create_preview_configuration(main={"size": (2592, 1944)}, transform = Transform(hflip=True, vflip=True))
            ...
            self.picam2.set_controls(self.config_disk)
            #self.picam2.set_controls(self.config_calib)
            #self.picam2.set_controls(self.config_pcb)
            h = int(self.h * 0.75)
            w = int((2592/1944) * h)
            self.qpicamera2 = QGlPicamera2(self.picam2, width = w, height = h, keep_ar = True)
...
# ----- CALIBRAR IMAGEN -------------------------
    ...
    def calibrate_camera(self):
        ...
        if ubi == None:
            ubi = "/home/deltaassysbot/Documentos/Delta-SVA/temp/calibrar/undist2/u2_28.png"
            #ubi = "/home/deltaassysbot/Desktop/Delta-SVA/temp/undistorted.png"
        print(ubi)
        pix = chess.pixel_mm(ubi, 11, 11, 10)                                                                                                       

        message = "Calibración Completa"
        detail = "Images: " + str(len(images)) + "\n"
        detail += "Pixel/mm: " + str(pix) + "\n"
