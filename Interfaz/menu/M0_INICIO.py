...
if os.name == 'nt':
    img_delta = os.path.join(path, 'Interfaz\imagenes\inicio.png')
    icono = os.path.join(path, 'Interfaz\imagenes\logo.png')
else:
    img_delta = os.path.join(path, 'Interfaz/imagenes/inicio.png')
    icono = os.path.join(path, 'Interfaz/imagenes/logo.png')
...
class INICIO(QWidget):
    ...
    # ----- INICIO WIDGET -----------------------------------------------------
        ...
        w = im.size().width()
        h = im.size().height()
        im_h = int(self.h * 0.85)
        im_w = (im_h * w) // h
        im = im.scaled(im_w, im_h, Qt.KeepAspectRatio)  # Escalar imagen al tamaño deseado conforme relación de pantalla
