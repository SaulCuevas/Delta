...
def genImage(CAMFolder, top):
    ...
    try:
        ...
        outline = load_layer(os.path.normpath(glob(os.path.join(path, '**/*.GM1'), recursive=True)[0]))
    putstr('. end\n')
    ...
    copper = load_layer(os.path.normpath(glob(os.path.join(path, '**/*.GBL'), recursive=True)[0]))
    putstr('.')
    mask = load_layer(os.path.normpath(glob(os.path.join(path, '**/*.GBS'), recursive=True)[0]))
    putstr('.')
    silk = load_layer(os.path.normpath(glob(os.path.join(path, '**/*.GBO'), recursive=True)[0]))
    putstr('. end\n')
    ...
    if top:
        ...
        # ctx.render_layer(copper, settings=RenderSettings(color=(1, 1, 1)), bgsettings=bg_settings)
        putstr('drawing mask ')
        putstr('.')
        ...
        # ctx.render_layer(copper, settings=RenderSettings(color=(1, 1, 1), mirror=True), bgsettings=bg_settings)
        putstr('drawing mask ')
        putstr('.')
        ctx.render_layer(mask, settings=metal_settings, bgsettings=bg_settings)
        putstr('.')
