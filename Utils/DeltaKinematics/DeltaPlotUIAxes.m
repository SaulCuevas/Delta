function DeltaPlotUIAxes(UIAxes, r, h, a, b, phi2, phi3, q1, q2, q3, px, py, pz)
...
cla(UIAxes);
plot3(UIAxes,TRx,TRy,TRz,'Color',"black",'Marker','o');
axis(UIAxes,'equal')
grid(UIAxes,'on')
hold(UIAxes,'on')

% Punto final
plot3(UIAxes,px,py,pz,'Color',"magenta",'Marker','x')
...
plot3(UIAxes,CC1x,CC1y,CC1z,'Color',"red",'Marker','o')
...
plot3(UIAxes,CC2x,CC2y,CC2z,'Color',"green",'Marker','o')
...
plot3(UIAxes,CC3x,CC3y,CC3z,'Color',"blue",'Marker','o')

UIAxes.XLim = [-a*1.5 a*1.5];
UIAxes.YLim = [-a*1.5 a*1.5];
UIAxes.ZLim = [-a a+b];

if(sqrt((a1x-b1x)^2 + (a1y-b1y)^2 + (a1z-b1z)^2) < b-1 || sqrt((a1x-b1x)^2 + (a1y-b1y)^2 + (a1z-b1z)^2) > b+1)
    fprintf('ERROR');
elseif(sqrt((a2x-b2x)^2 + (a2y-b2y)^2 + (a2z-b2z)^2) < b-1 || sqrt((a2x-b2x)^2 + (a2y-b2y)^2 + (a2z-b2z)^2) > b+1)
    fprintf('ERROR');
elseif(sqrt((a3x-b3x)^2 + (a3y-b3y)^2 + (a3z-b3z)^2) < b-1 || sqrt((a3x-b3x)^2 + (a3y-b3y)^2 + (a3z-b3z)^2) > b+1)
    fprintf('ERROR');
end